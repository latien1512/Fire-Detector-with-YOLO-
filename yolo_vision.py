...
print("\n▶️ YOLO Detection system started. Press 'q' to stop.\n")
    ...
    while True:
        ...
        if highest_hazard_in_frame != "SAFE":
            ...
            if hazard_start_time is None:
                ...
                print(f"Detected {current_hazard_type}... Starting 10-second timer.")
            ...
            if hazard_start_time is not None:
                ...
                if time_since_last_seen <= TOLERANCE_TIME:
                    ...
                    print("Complete signal loss -> Canceling timer.")
                    hazard_start_time = None
                    cancel_msg_time = current_time
                    update_status("SAFE") 

        # ================= DISPLAY AND SAVE IMAGE IN REAL-TIME =================
        if hazard_start_time is not None:
            time_left = max(0, REQUIRED_TIME - (current_time - hazard_start_time))
            cv2.putText(frame, f"Verifying {current_hazard_type}: {time_left:.1f}s", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
        elif current_time - cancel_msg_time < 3.0:
            cv2.putText(frame, "No signal detected -> Canceling timer", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3)

        cv2.imshow("Smart PCCC System - YOLO Detection", frame)
        
        # --- SAVE CURRENT IMAGE TO RAM FOR EMAIL NOTIFICATION ---
        try:
